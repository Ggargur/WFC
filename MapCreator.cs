...
namespace WaveFunction
{
    public class MapCreator : SingletonBase<MapCreator>
    {
        ...
        public Dictionary<Direction, Cell> GetNeighbors(Cell cell)
        {
            ...
            foreach (var dir in AllDirections)
            {
                ...
                if (IsInBounds(neighborPos))
                    neighbors.TryAdd(dir, _grid[GetIndex(neighborPos)]);
            }
        ...
        private static IEnumerable<Cell> GetNextOptions(IEnumerable<Cell> chunk)
        {
            ...
            if (!allOptions.Any()) return allOptions;
        ...
        private void InitGrid()
        {
            ...
            {
                _grid[GetIndex(i, j, k)] = new Cell(new Vector3Int(i, j, k), _systemRandom);
            }
        }
        ...
        private async Task SolveChunk(List<Cell> chunk)
        {
            for (int attempt = 0; attempt < maxChunkRetries; attempt++)
            {
                ...
                while (chunk.Any(c => !c.IsCollapsed))
                {
                    ...
                    if (!CollapseInChunk(next, chunkStack))
                    {
                        failed = true;
                        break;
                    }
        ...
        private bool CollapseInChunk(Cell next, Stack<Cell> chunkStack)
        {
            while (true)
            {
                if (next == null)
                ...
                // Backtrack somente dentro do chunk
                next = chunkStack.Pop();
                _instancedCells.TryPop(out _);
                next.Uncollapse();
            }
        }
        
        private bool NeighborsHaveMistake(Cell cell)
        {
            return cell.Neighbors.Values.Any(n => !n.IsCollapsable && !n.IsCollapsed);
        }
